...
?>
		...
		<?php printf(__('<cite class="fn">%s</cite> <span class="says">says:</span>'), strtoupper(get_comment_author_link())) ?>
		</div>
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields =  array(
		...
		'email'  => '<p class="comment-form-email"><label for="email">' . __( 'Email (optional):' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
		            '<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>',
		'url'    => '<p class="comment-form-url"><label for="url">' . __( 'Website (optional):' ) . '</label>' .
		            '<input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p>',
		//'send_mail' => '<div class="comment-mail"><input id="send_mail" name="send_mail" type="checkbox" checked /><label for="send_mail" class="comment-lbl-checkbox">Notify me of follow-up comments via email.</label></div>'
	);
	...
	$defaults = array(
		...
		'comment_notes_before' => '<p class="comment-notes">' . __( 'There are no comments for this journal entry. To create a new comment, use the form below.' ) . ( $req ? $required_text : '' ) . '</p>',
		//'comment_notes_after'  => '<p class="form-allowed-tags">' . sprintf( __( 'You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes: %s' ), ' <code>' . allowed_tags() . '</code>' ) . '</p>',
		'id_form'              => 'commentform',
		'id_submit'            => 'submit',
		'title_reply'          => __( 'Post New Comments' ),
		'title_reply_to'       => __( 'Leave a Reply to %s' ),
		'cancel_reply_link'    => __( 'Cancel reply' ),
		'label_submit'         => __( 'Create Post' ),
	);
