...
if ( !@mysql_select_db( DB_NAME, $dbh ) ) {
	...
	$bodyContent .= "Here is your Daily Report for the Get Help section of your website <br/>";
	
	$bodyContent .= '<table border="1"><tr>';
	$fields = getForm('SELECT * FROM wp_ninja_forms_fields WHERE `form_id` = 3 ORDER BY `order` ASC', $dbh);
	...
	$records = getForm("SELECT * FROM wp_ninja_forms_subs WHERE `form_id` = 3 AND date_updated > '".$yesterday." 00:00:00' AND date_updated < '".$today." 00:00:00' ORDER BY `date_updated` DESC ", $dbh);
	...
	$mail->AddAddress('dhalfenberg@midnightmission.org', 'Don Halfenberg');
	//$mail->AddAddress('chris@liquidmindcreative.com', "Chris");
	//$mail->AddAddress('cann@liquidmindcreative.com', 'Can');
	

	
	$mail->Subject = '[Midnight Mission] Get Help Daily Report '. $yesterday ;
	if($numberItems == 1) {
